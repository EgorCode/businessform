...
Этот документ содержит полный bash скрипт для автоматического развертывания проекта BizStartMaster на VPS сервере с Ubuntu 22. Скрипт выполняет все необходимые действия: установку системных зависимостей, настройку веб-сервера, развертывание приложения и настройку мониторинга.

## Структура скрипта

Скрипт `deploy.sh` включает следующие функции:

### 1. Установка системных зависимостей
- Обновление системы
- Установка Apache2
- Установка Node.js 18.x
- Установка Git и необходимых утилит
- Настройка firewall

### 2. Настройка Apache2
- Включение необходимых модулей (proxy, proxy_http, headers, rewrite)
- Создание конфигурации виртуального хоста
- Настройка проксирования запросов к Node.js приложению
...
- Создание файла сервиса для автоматического запуска
- Настройка переменных окружения
- Включение автозапуска сервиса

### 5. Настройка переменных окружения
- Создание production-конфигурации
- Настройка безопасных ключей
- Конфигурация AI API (Gemini)
...
```bash
# С параметрами
sudo ./deploy.sh "https://github.com/your-username/bizstartmaster.git" "your-domain.com" "admin@your-domain.com" "your-gemini-api-key"
```

### Параметры скрипта

1. `REPO_URL` - URL репозитория проекта (по умолчанию: https://github.com/your-username/bizstartmaster.git)
2. `DOMAIN` - Домен или IP адрес сервера (по умолчанию: localhost)
3. `EMAIL` - Email администратора (по умолчанию: admin@localhost)
4. `GEMINI_API_KEY` - API ключ для Google Gemini (опционально)

## Что делает скрипт

1. **Подготовка сервера**:
   - Обновляет систему
   - Устанавливает Apache2, Node.js 18.x, Git
   - Настраивает firewall

2. **Настройка веб-сервера**:
   - Конфигурирует Apache2 для работы с Node.js приложением
   - Настраивает проксирование API запросов
   - Включает сжатие и заголовки безопасности
...
4. **Настройка сервисов**:
   ...
   - Конфигурирует резервное копирование

5. **Мониторинг и обслуживание**:
   - Настраивает ротацию логов
   - Создает скрипты обновления и бэкапа
...
2. **Настройте SSL сертификат** (рекомендуется):
   ```bash
...
2. **Используйте надежные пароли** и SSH ключи
