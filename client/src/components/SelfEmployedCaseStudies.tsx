...
const staticCaseStudies: CaseStudy[] = [
  ...
  },
  {
    ...
    niche: "Ремесло и услуги",
    icon: Hammer,
    problem: "Артем чинит обувь и делает ключи. Поток клиентов растет, и ему нужно нанять помощника. Но самозанятые не могут нанимать сотрудников.",
    journey: [
      "Артем проходит тест на портале: «Планируете штат?». Ответ: «Да».",
      "Система предупреждает: «Внимание! Самозанятость ограничивает найм. Рассмотрите ИП на НПД или Патенте».",
      "Артем видит расчет: Патент будет выгоднее при обороте выше 100 тыс. в месяц."
    ],
    result: "Артем выбирает ИП на НПД как переходный этап, чтобы сохранить низкую ставку (4/6%), но иметь статус ИП для аренды помещения в ТЦ.",
    subscription: "max",
    features: ["Калькулятор Патента vs НПД", "Конструктор трудового договора (для ИП)", "Календарь отчетности"],
    taxRate: "4% / 6%",
    clientType: "Физлица и аренда"
  },
  {
    id: "elena-rent",
    name: "Елена",
    ...
    role: "Сдача квартиры",
    niche: "Аренда недвижимости",
    icon: Home,
    problem: "Елена сдает квартиру посуточно командировочным. Ей нужны закрывающие документы для бухгалтерии клиентов.",
    journey: [
      "Елена скачивает шаблон «Акт приема-передачи» из раздела Документы.",
      "Она настраивает авто-чеки: как только деньги падают на карту, чек отправляется на email гостя.",
      "В конце года система показывает ей лимит дохода (2.4 млн), чтобы она не «вылетела» с режима."
    ],
    result: "Елена спокойно работает с командировочными, предоставляя им официальные чеки, что повышает её рейтинг на площадках бронирования.",
    subscription: "lite",
    features: ["Генератор актов", "Контроль лимита 2.4 млн", "Авто-отправка чеков"],
    taxRate: "6%",
    clientType: "Физлица и юрлица"
  },
  {
    id: "mikhail-copy",
    name: "Михаил",
    ...
    role: "Копирайтер и контент",
    niche: "Контент и реклама",
    icon: BookOpen,
    problem: "Михаил работает с иностранными заказчиками (Казахстан, Армения). Он не знает, как платить налог с валютных поступлений.",
    journey: [
      "Михаил читает статью в Базе Знаний: «ВЭД для самозанятых: просто о сложном».",
      "Он узнает, что налог платится по курсу ЦБ на день поступления средств.",
      "Использует валютный калькулятор на портале, чтобы сразу откладывать нужную сумму на налог."
    ],
    result: "Михаил легализовал свой доход из-за рубежа и не боится блокировок карт банком по 115-ФЗ.",
    subscription: "max",
    features: ["Валютный калькулятор", "Справки для банка (115-ФЗ)", "Консультации юриста"],
    taxRate: "0% / 6%",
    clientType: "Иностранные заказчики"
  }
];
